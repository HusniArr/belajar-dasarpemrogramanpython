...
class Engine:
    ...
    def drive_sound(self):
        if self.engine_name == ENGINE_ELECTRIC:
            return "eeeeaahhhzz"
        if self.engine_name == ENGINE_PETROL:
            return "vroom vroooommmm"
        if self.engine_name == ENGINE_DIESEL:
            return "vroom vrooooommm vroooommm"

class ElectricCar(Vehicle, Engine):
    def __init__(self):
        Vehicle.__init__(self, name="electric car", number_of_wheels=4)
